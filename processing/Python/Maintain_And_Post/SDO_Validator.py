...
WHERE M.sdo_table_name = '{2}'
AND M.sdo_column_name = '{3}'
AND SDO_GEOM.VALIDATE_GEOMETRY_WITH_CONTEXT(A.{3}, M.sdo_diminfo) <> 'TRUE'"""

sdoVAL_A = """SELECT A.{0}
FROM  {1}.A{4} A,
MDSYS.SDO_GEOM_METADATA_TABLE M
WHERE M.sdo_table_name = 'A{2}'
AND M.sdo_column_name = '{3}'
AND SDO_GEOM.VALIDATE_GEOMETRY_WITH_CONTEXT(A.{3}, M.sdo_diminfo) <> 'TRUE'"""
